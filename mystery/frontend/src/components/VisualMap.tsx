...
import { AGENT_TILE_SCALE, MAP_GRID, mapImageForScale, mapImageTiles } from "../map/mapAssets";
...
export default function VisualMap({
  ...
  /** The selected player-discovered clue. These only decorate known links. */
  ...
  const internalThreshold =
    data.map.zoom_image_tiles?.threshold ?? data.map.zoom_image?.threshold ?? Infinity;
      ...
      >
        ...
        >
          <div className="visual-map-image-frame">
            {(() => {
              ...
              return (
                <img
                  ...
                  src={mapImageForScale(data.map, view.scale)}
                  alt="Village map"
                  draggable={false}
                  style={artStyle}
                />
            ...
            <CaseAtmosphere caseId={data.case_id} weather={data.weather} />
          </div>
        </div>
        ...
        {data.locations.map((loc) => {
          ...
          const evidenceLinked = evidenceLocationIds.includes(loc.location_id);
          ...
          return (
            <div key={loc.location_id}>
              {bounds && (
                <div
                  className={`map-loc-bounds ${selected ? "selected" : ""} ${evidenceLinked ? "evidence-linked" : ""} ${
                    selected || evidenceLinked || hoveredLocationId === loc.location_id ? "visible" : ""
              ...
              {showLabels && (
                <button
                  className={`map-loc-label ${selected ? "selected" : ""} ${evidenceLinked ? "evidence-linked" : ""}`}
                  style={{
        ...
        {pins.map((pin) => (
          <div
            ...
            className={`map-agent-anchor ${evidenceAgentIds.includes(pin.agent.agent_id) ? "evidence-linked" : ""}`}
            style={{
              left: pct(pin.x, width),
              top: pct(pin.y, height),
              "--map-evidence-scale": 1 / view.scale,
            } as React.CSSProperties}
          >
            <AgentSprite
              agent={pin.agent}
