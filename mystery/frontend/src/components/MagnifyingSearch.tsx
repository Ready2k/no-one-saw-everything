...
type SearchTool = "lens" | "raking" | "uv";
type SceneView = "wide" | "left" | "centre" | "right" | "custom";
...
const SEARCH_TOOLS: { id: SearchTool; label: string; readout: string }[] = [
  ...
  { id: "uv", label: "UV lamp", readout: "Reveals cleaned or biological trace" },
];

/**
 * This is deliberately based only on clue titles already supplied in the
 * player-safe inspection projection.  It never exposes hidden case truth;
 * it merely makes the physical search method matter across every case.
 */
function requiredToolFor(clue: ClueHotspot): SearchTool {
  if (clue.search_tool) return clue.search_tool;
  const title = clue.title.toLowerCase();
  if (/(blood|stain|damp|smudge|residue|fiber|hair|fluid|cleaned)/.test(title)) return "uv";
  if (/(footprint|shoe|boot|mud|scratch|mark|impression|scuff)/.test(title)) return "raking";
  return "lens";
}
...
export function MagnifyingSearch({
  ...
  const [sceneView, setSceneView] = useState<SceneView>("wide");
  ...
  // detail under the beam; it avoids treating a flat coloured circle as light.
  const renderSceneImage = (className = "") => (
    <div
      className={className}
      style={{
        width: `${bgSizeX}%`,
  ...
  const sceneStyle = {
    ...
    "--search-light-x": `${mousePos?.x ?? dim.w / 2}px`,
    "--search-light-y": `${mousePos?.y ?? dim.h / 2}px`,
  ...
  const handleMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const found = adjustedClues.find((c) => {
      ...
      return dist <= c.radius && isRelativeSize && requiredToolFor(c) === activeTool;
    });
  ...
  const handleClick = () => {
    ...
    if (!mousePos || dim.w === 0) return;
    const pctX = (mousePos.x / dim.w) * 100;
    const pctY = (mousePos.y / dim.h) * 100;
    const nearby = adjustedClues.find((c) => {
      const dx = c.x - pctX;
      const dy = (c.y - pctY) / (dim.h > 0 ? dim.w / dim.h : 1);
      return Math.sqrt(dx * dx + dy * dy) <= c.radius;
    });
    if (nearby) {
      const needed = SEARCH_TOOLS.find((item) => item.id === requiredToolFor(nearby))?.label;
      setToolFeedback(`No clear result. Try ${needed}.`);
    }
  };
  ...
  const selectSceneView = (view: Exclude<SceneView, "custom">) => {
    sfx.lensAdjust();
    setSceneView(view);
    if (view === "wide") {
      setZoomLevel(1);
    ...
    setZoomLevel(view === "centre" ? 1.8 : 1.65);
    setPanOffset({
      x: view === "left" ? -21 : view === "right" ? 21 : 0,
      y: 0,
    });
  };
  ...
  return (
    <>
      {!isPortrait && (
        <header className="forensic-scene-head">
          <div>
            <span className="cafe-kicker">Scene examination · {sceneTitle || "Location search"}</span>
            <p>Survey the scene, then work each surface with the appropriate tool.</p>
          </div>
    ...
    >
      ...
      {!isPortrait && (
        <>
          ...
          <div className="zoom-controls">
            <button
              onClick={(e) => { e.stopPropagation(); sfx.lensAdjust(); setSceneView("custom"); setZoomLevel(Math.max(1, zoomLevel - 0.5)); }}
            ...
            <button
              onClick={(e) => { e.stopPropagation(); sfx.lensAdjust(); setSceneView("custom"); setZoomLevel(Math.min(3, zoomLevel + 0.5)); }}
      ...
      {!isPortrait && activeTool !== "lens" && dim.w > 0 && (
        <div className={`scene-light-reveal reveal-${activeTool}`} aria-hidden="true">
          {renderSceneImage("scene-light-reveal-image")}
          <div className="scene-light-tint" />
          <span className="scene-light-reticle" />
        </div>
      )}

      {!isPortrait && (
        <div className={`scene-tool-in-hand scene-tool-${activeTool}`} aria-hidden="true">
          <span className="scene-tool-beam" />
          <span className="scene-tool-body" />
        </div>
      )}
        ...
        adjustedClues.map((c, i) => (
          <button
            ...
            aria-label={`Search spot ${i + 1} of ${adjustedClues.length}`}
              onClick={(e) => {
                e.stopPropagation();
              if (requiredToolFor(c) === activeTool) {
                onDiscover(c.clue_id);
              } else {
                const needed = SEARCH_TOOLS.find((item) => item.id === requiredToolFor(c))?.label;
                setToolFeedback(`No clear result. Try ${needed}.`);
              }
            }}
          />
      ...
      {mousePos && dim.w > 0 && activeTool === "lens" && (
        <div
          ...
          <div className="lens-content">
            <div
              style={{
                ...
                width: dim.w,
                height: dim.h,
                left: LENS_SIZE / 2 - mousePos.x * ZOOM,
                top: LENS_SIZE / 2 - mousePos.y * ZOOM,
      ...
      {!isPortrait && (
        <>
          <div className="forensic-scene-controls">
            <div className="forensic-control-group">
              <span>Camera</span>
              {(["wide", "left", "centre", "right"] as const).map((view) => (
                <button key={view} className={sceneView === view ? "active" : ""} onClick={() => selectSceneView(view)}>
                  {view === "wide" ? "Wide" : view === "centre" ? "Centre" : view[0].toUpperCase() + view.slice(1)}
                </button>
              ))}
            </div>
            <div className="forensic-control-group">
              ...
              {SEARCH_TOOLS.map((tool) => (
                <button key={tool.id} className={activeTool === tool.id ? "active" : ""} onClick={() => { if (activeTool !== tool.id) sfx.lensAdjust(); setActiveTool(tool.id); setToolFeedback(null); }}>
                  {tool.label}
                </button>
              ))}
            </div>
          </div>
          <div className="forensic-scene-readout panel"><div><span className="cafe-kicker">{toolFeedback ? "Tool reading" : "Select a surface"}</span><p>{toolFeedback || SEARCH_TOOLS.find((tool) => tool.id === activeTool)?.readout}</p></div></div>
        </>
      )}
    </>
  );
