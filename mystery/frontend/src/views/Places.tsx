import { useEffect, useState } from "react";
import { api } from "../api";
import { useWorld } from "../App";
import type { InspectResult, LocationPublic, ClueHotspot, CluePublic } from "../types";
import { ClueCard } from "./shared";
import { MagnifyingSearch } from "../components/MagnifyingSearch";
import { sfx } from "../sfx";
import LocationTransition, {
  shouldPlayLocationTransition,
} from "../components/LocationTransition";
import { getMapInfo } from "../map/mapInfo";
import type { MapReplayData } from "../types";
import { HobbsCafeScene } from "../components/HobbsCafeScene";
import { CafeKitchenScene } from "../components/CafeKitchenScene";
import { CafeStorageRoomScene } from "../components/CafeStorageRoomScene";
import { sceneArtFor } from "../sceneArt";

const PRIVATE_PLACE_HELP = "Private marks a residence, back room, or other non-public area. You may still search it during this investigation; it is context, not a locked door. A new lead can make a previously hidden detail discoverable.";

export default function Places() {
  const { locations, caseOverview } = useWorld();
  const [mapData, setMapData] = useState<MapReplayData | null>(null);
  const [selected, setSelected] = useState<string | null>(null);
  const [result, setResult] = useState<InspectResult | null>(null);
  const [busy, setBusy] = useState(false);
  const [transitionLoc, setTransitionLoc] = useState<LocationPublic | null>(null);
  const [newlyFound, setNewlyFound] = useState<Set<string>>(new Set());

  useEffect(() => {
    getMapInfo().then(setMapData).catch(() => setMapData(null));
  }, []);

  const inspect = async (locationId: string) => {
    const loc = locations.find((l) => l.location_id === locationId);
    if (loc && shouldPlayLocationTransition(locationId)) setTransitionLoc(loc);
    setSelected(locationId);
    setNewlyFound(new Set());
    setBusy(true);
    try {
      const res = await api.inspect(locationId);
      if (res.new_clues?.length) {
        sfx.evidenceFound();
      }
      setResult(res);
    } finally {
      setBusy(false);
    }
  };

  const handleDiscover = async (clueId: string) => {
    if (!result) return;
    try {
      const discoveredClue = await api.discoverClue(clueId);
      sfx.evidenceFound();
      setNewlyFound((previous) => new Set(previous).add(clueId));
      setResult((prev: InspectResult | null) => {
        if (!prev) return prev;
        return {
          ...prev,
          hidden_clues: prev.hidden_clues.filter((c: ClueHotspot) => c.clue_id !== clueId),
          known_clues: [...prev.known_clues, discoveredClue],
        };
      });
    } catch (e) {
      console.error(e);
    }
  };

  return (
    <div className="places">
      {transitionLoc && (
        <LocationTransition
          location={transitionLoc}
          onDone={() => setTransitionLoc(null)}
        />
      )}
      <div className="place-list panel">
        <h2>Search the village</h2>
        <p className="muted small">
          Inspect a place to search it for physical evidence. Some things only
          make sense once you know what to look for.
        </p>
        <p className="places-starting-lead">
          <strong>First lead:</strong> start at {caseOverview.discovery_location.name}, then follow any clue it points elsewhere.
        </p>
        {locations.map((l) => (
          <button
            key={l.location_id}
            className={`place ${selected === l.location_id ? "active" : ""}`}
            onClick={() => inspect(l.location_id)}
            aria-describedby={l.visibility_type === "private" ? `private-place-help-${l.location_id}` : undefined}
          >
            <span className="place-name">{l.name}</span>
            {l.visibility_type === "private" && (
              <span className="private-place-badge">
                <span className="badge ambiguous" aria-hidden="true">private</span>
                <span id={`private-place-help-${l.location_id}`} className="private-place-tooltip" role="tooltip">
                  <strong>About private places</strong>
                  {PRIVATE_PLACE_HELP}
                </span>
              </span>
            )}
          </button>
        ))}
      </div>
      <div className="place-detail">
        {!result && <p className="muted">Pick a location to search it.</p>}
        {busy && <p className="muted">Searching…</p>}
        {result && !busy && (
          <div className="places-scroll-container">
            <h2>{result.location.name}</h2>
            
            {result.location.location_id === "loc_hobbs_cafe" ? (
              <HobbsCafeScene imageUrl={sceneArtFor(caseOverview.case_id, result.location.location_id, result.location.illustration)} hiddenClues={result.hidden_clues || []} onDiscover={handleDiscover} />
            ) : result.location.location_id === "loc_cafe_kitchen" ? (
              <CafeKitchenScene imageUrl={sceneArtFor(caseOverview.case_id, result.location.location_id, result.location.illustration)} hiddenClues={result.hidden_clues || []} onDiscover={handleDiscover} />
            ) : result.location.location_id === "loc_cafe_storage" ? (
              <CafeStorageRoomScene imageUrl={sceneArtFor(caseOverview.case_id, result.location.location_id, result.location.illustration)} hiddenClues={result.hidden_clues || []} onDiscover={handleDiscover} />
            ) : (
              <MagnifyingSearch
                 bounds={sceneArtFor(caseOverview.case_id, result.location.location_id, result.location.illustration)
                   ? { x: 0, y: 0, width: 100, height: 100 }
                   : result.location.map_bounds}
                 hiddenClues={result.hidden_clues || []}
                 imageUrl={sceneArtFor(caseOverview.case_id, result.location.location_id, result.location.illustration) || mapData?.map.image}
                 mapWidth={sceneArtFor(caseOverview.case_id, result.location.location_id, result.location.illustration) ? 100 : mapData?.map.width}
                 mapHeight={sceneArtFor(caseOverview.case_id, result.location.location_id, result.location.illustration) ? 100 : mapData?.map.height}
                 isIllustration={Boolean(sceneArtFor(caseOverview.case_id, result.location.location_id, result.location.illustration))}
                 locationId={result.location.location_id}
                 sceneTitle={result.location.name}
                 onDiscover={handleDiscover}
              />
            )}

            <div className="search-status" style={{ marginBottom: "1rem" }}>
               <p style={{ margin: 0 }}><strong>Search status:</strong> {(result.known_clues.length)} / {(result.known_clues.length + (result.hidden_clues?.length || 0))} clues found</p>
            </div>

            <h3>Found evidence</h3>
            {result.known_clues.length > 0 ? (
               <div className="found-evidence-list">
                 {result.known_clues.map((c: CluePublic) => (
                   <ClueCard key={c.clue_id} clue={c} isNew={newlyFound.has(c.clue_id)} />
                 ))}
               </div>
            ) : (
               <p className="muted">Nothing found yet.</p>
            )}

            <h3 style={{ marginTop: "1rem" }}>Notes</h3>
            <p>{result.location.description}</p>
            {result.hint && <p className="hint-text">{result.hint}</p>}
          </div>
        )}
      </div>
    </div>
  );
}
