...
// The canonical editor world is centred on the same village art used by the
// game. Every cell has a geometry-matched roofed/cutaway pair so locations
// spanning the A2/B2 boundary change view consistently.
export const CURRENT_VILLAGE_PREFIX = "/art/town/tiles_3x3_hd/current_village_v3_neutral";
const CURRENT_VILLAGE_ASSET_VERSION = "20260802-farmhouse-footprint-v6";
const currentVillageUrl = (filename: string) =>
  `${CURRENT_VILLAGE_PREFIX}/${filename}?v=${CURRENT_VILLAGE_ASSET_VERSION}`;
export const HD_B2_EXTERNAL_URL = currentVillageUrl("town_overworld_B2_current_village_v3_neutral.avif");
export const HD_B2_INTERNAL_URL = currentVillageUrl("town_overworld_B2_current_village_v3_neutral_cutaway.avif");
export const LIVING_TOWN_V2_TILE_URLS: Partial<Record<string, string>> = {
  A1: currentVillageUrl("town_overworld_A1_current_village_v3_neutral.avif"),
  A2: currentVillageUrl("town_overworld_A2_current_village_v3_neutral.avif"),
  A3: currentVillageUrl("town_overworld_A3_current_village_v3_neutral.avif"),
  B1: currentVillageUrl("town_overworld_B1_current_village_v3_neutral.avif"),
  B2: HD_B2_EXTERNAL_URL,
  B3: currentVillageUrl("town_overworld_B3_current_village_v3_neutral.avif"),
  C1: currentVillageUrl("town_overworld_C1_current_village_v3_neutral.avif"),
  C2: currentVillageUrl("town_overworld_C2_current_village_v3_neutral.avif"),
  C3: currentVillageUrl("town_overworld_C3_current_village_v3_neutral.avif"),
};

export const HD_TILE_CELLS = (["A", "B", "C"] as const).flatMap((row) =>
  ([1, 2, 3] as const).map((col) => `${row}${col}`)
);
export const LIVING_TOWN_V2_INTERNAL_TILE_URLS: Partial<Record<string, string>> =
  Object.fromEntries(
    HD_TILE_CELLS.map((cell) => [
      cell,
      currentVillageUrl(`town_overworld_${cell}_current_village_v3_neutral_cutaway.avif`),
    ])
  );
...
export function hdDefaultTileUrl(cell: string, view: MapView): string {
  const livingTownUrl = view === "internal"
    ? LIVING_TOWN_V2_INTERNAL_TILE_URLS[cell]
    : LIVING_TOWN_V2_TILE_URLS[cell];
...
export const UNDERLAY_SOURCES: UnderlaySourceDef[] = [
  { id: "town_tiles_hd", label: "Town overworld HD (3×3 tiles)", tiles: HD_3X3_TILES },
  {
    ...
    tiles: [{ url: "/art/town/town_canonical_v2_overworld_day.avif", x: 0, y: 0, w: 192, h: 144 }]
  },
  {
    id: "town_day",
    label: "Town canonical v1 (day, centre)",
    tiles: [{ url: "/art/town/town_canonical_v1_day.avif", ...CENTRE_THIRD }]
  }
];
