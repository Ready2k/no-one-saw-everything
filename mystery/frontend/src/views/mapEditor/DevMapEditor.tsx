...
// Map Replay changes from its roofed overview to its roofless cutaway at
// 3.2× zoom. The editor camera uses pixels-per-tile, so compare its scale
// with the fitted overview scale rather than an absolute camera value.
const INTERIOR_ZOOM_THRESHOLD = 3.2;
...
export default function DevMapEditor() {
      ...
      underlays:
        st.underlaySource === "none"
          ...
          : (UNDERLAY_SOURCES.find((u) => u.id === st.underlaySource)?.tiles || [])
              .map((t) => {
                // Mosaic cells resolve per view (each pairs external/interior
  ...
  useEffect(() => {
    ...
    urls.push(
      HD_B2_INTERNAL_URL,
      ...Object.values(LIVING_TOWN_V2_INTERNAL_TILE_URLS).filter(
  ...
  const fittedOverviewScale = () => {
    const lay = layoutRef.current;
    const { w, h } = sizeRef.current;
    if (!lay || !w || !h) return 1;
    return clamp(
      Math.min((w - 40) / lay.grid.cols, (h - 40) / lay.grid.rows),
      MIN_SCALE,
      MAX_SCALE
    );
  };

  const syncMapViewForZoom = (scale: number) => {
    // A selected location's inspection art is a separate scene. Only the
    // village overworld behaves like the runtime map and swaps on zoom.
    if (stateRef.current?.underlaySource !== "town_tiles_hd") return;
    setMapView(
      scale / fittedOverviewScale() >= INTERIOR_ZOOM_THRESHOLD ? "internal" : "external"
    );
  };
      ...
      <div className="main-editor-layout">
        ...
        <aside className="left-sidebar">
          ...
          <div className="control-group">
            ...
            <div className="view-note">
              Town overworld HD follows Map Replay automatically: internal cutaway at 3.2× overview zoom.
            </div>
          </div>
