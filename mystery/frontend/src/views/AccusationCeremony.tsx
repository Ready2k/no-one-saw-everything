...
export default function AccusationCeremony({
  ...
  const confessionVideoRef = useRef<HTMLVideoElement>(null);
  const [confessionEnded, setConfessionEnded] = useState(false);
  const [confessionMuted, setConfessionMuted] = useState(true);

  // Each authored case has its own reconstruction. Procedural cases keep the
  // data-driven truth timeline rather than being shown footage from a different
  // solution.
  const confessionVideos: Record<string, string> = {
    case_001: "/cinematics/case-001-confession.mp4",
    case_002: "/cinematics/case-002-confession.mp4",
    case_003: "/cinematics/case-003-confession.mp4",
    case_004: "/cinematics/case-004-confession.mp4",
    case_005: "/cinematics/case-005-confession.mp4",
    case_006: "/cinematics/case-006-confession.mp4",
    case_007: "/cinematics/case-007-confession.mp4",
    case_010: "/cinematics/case-010-confession.mp4",
  };
  const confessionVideo = confessionVideos[caseOverview.case_id] ?? null;
  ...
  if (result.killer_correct) {
    steps.push("killer");
    if (confessionVideo) steps.push("confession");
    steps.push("timeline");
  }
  ...
  return (
    <div className="ceremony-container">
      ...
      <div className={`ceremony-content step-${step}`}>
        ...
        {step === "confession" && confessionVideo && (
          <section className="ceremony-confession fade-in" aria-labelledby="confession-title">
            <div className="confession-heading">
              <p className="ceremony-suspense-kicker">CONFESSION · RECONSTRUCTION</p>
              <h2 id="confession-title">How {result.true_killer_name} did it</h2>
            </div>
            <div className="confession-video-frame">
              <video
                ref={confessionVideoRef}
                className="confession-video"
                src={confessionVideo}
                autoPlay
                muted={confessionMuted}
                playsInline
                preload="metadata"
                onEnded={() => setConfessionEnded(true)}
                aria-label={`${result.true_killer_name}'s confession and reconstruction of the murder`}
              >
                Your browser cannot play this reconstruction video.
              </video>
            </div>
            <div className="confession-actions">
              <button
                type="button"
                onClick={() => {
                  const video = confessionVideoRef.current;
                  if (!video) return;
                  const nextMuted = !confessionMuted;
                  video.muted = nextMuted;
                  setConfessionMuted(nextMuted);
                  if (!nextMuted) void video.play();
                }}
              >
                {confessionMuted ? "Enable sound" : "Mute sound"}
              </button>
              <button
                type="button"
                onClick={() => {
                  ...
                  if (!video) return;
                  video.currentTime = 0;
                  setConfessionEnded(false);
                  void video.play();
                }}
              >
                Replay reconstruction
              </button>
              {confessionEnded && <span className="confession-complete">Reconstruction complete</span>}
            </div>
          </section>
        )}
