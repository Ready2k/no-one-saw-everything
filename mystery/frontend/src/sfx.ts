...
export const sfx = {
  ...
  click() {
    ...
    const c = ctx();
    if (!c) return;
    // Body stays above 380 Hz: a small speaker cannot reproduce a 130 Hz decay
    // and distorts it into a buzz instead. Crispness lives at 2-8 kHz.
    thump(c, { freqFrom: 2300, freqTo: 1400, duration: 0.02, volume: 0.07, type: "square" });
    thump(c, { at: c.currentTime + 0.006, freqFrom: 820, freqTo: 380, duration: 0.035, volume: 0.045 });
  },
