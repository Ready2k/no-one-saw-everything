...
export interface AgentPublic {
  ...
  /**
   * In-voice lines to play while an answer is being generated, so a slow turn
   * reads as a person thinking rather than a spinner. Sent with the agent so
   * showing one costs no round trip.
   *
   * They are deliberately affect-neutral and identical at every pressure level:
   * hesitation is real evidence in this game, and a stall that sounded evasive
   * would be a tell invented by GPU load. Never render them as a suspect's
   * *answer* — they are a held breath, and the real line replaces them.
   */
  stall_fillers: string[];
}
...
export type WeatherCondition = "clear" | "overcast" | "rain" | "fog" | "wind" | "storm" | "snow" | "slush";
export type WeatherIntensity = "light" | "moderate" | "heavy";
export type WindStrength = "calm" | "breezy" | "strong";
export interface CaseWeather { condition: WeatherCondition; intensity: WeatherIntensity; wind: WindStrength; }
...
export interface MapReplayData {
  ...
  map: {
    ...
    // Same-frame close zoom asset for maps which are authored as one image
    // rather than a mosaic. Used for roofless interior cutaways.
    zoom_image?: { threshold: number; url: string };
    width: number;
    height: number;
...
export interface CluePublic {
  ...
  /** Where the player recovered this clue, when it was found by inspecting a place. */
...
export interface ChallengeSuggestion {
  ...
  // "" for a testimony-based suggestion (the evidence is another claim, not
  // a clue) — set for exactly one of evidence_clue_id / evidence_claim_id.
...
export interface ClueHotspot {
  ...
  title: string;
  /** Authored forensic method. Legacy cases retain the safe title-based fallback. */
  search_tool?: "lens" | "raking" | "uv";
}
...
export interface Config {
  ...
  dev_map_editor_enabled: boolean;
...
export interface LlmSettingsEffective {
  ...
  base_url: string | null;
  model: string | null;
  /** Settings are filled in. Says nothing about whether the host answers. */
  configured: boolean;
  /** Whether the host actually responded. null = probe still running, or no
   *  network provider to check. */
  reachable: boolean | null;
  /** Human-readable reason the endpoint could not be reached. */
  health_detail: string | null;
  fallback_reason: string | null;
  detected_source: string | null;
  dialogue_enabled: boolean;
  /** The session already hit a provider error and dropped to the no-LLM path
   *  for the rest of this session (spec §11), even if the host is back now. */
