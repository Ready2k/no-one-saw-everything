...
def project_agent(agent: Agent) -> dict[str, Any]:
    ...
    return {
        ...
        # In-voice lines the UI plays while a rewrite generates, so a slow turn
        # reads as a person thinking rather than a machine. Sent with the agent
        # rather than per-question so showing one costs no round trip — and
        # they carry no case truth, no pressure signal and no evidence, which is
        # exactly why they are safe to hand over up front. See
        # dialogue_processor.stall_fillers_for.
        "stall_fillers": stall_fillers_for(agent),
    }
...
def project_map_location(loc: Location, case_id: str | None = None) -> dict[str, Any]:
    ...
    if case_id:
        ...
        authored = (config or {}).get("location_visuals", {}).get(loc.location_id)
        if authored:
            # Case-art maps have their own pixel space.  Use the matching
            # authored bounds for both the visible location region and the
            # replay tracks; canonical fallback coordinates can be several
            # times larger than the case-art image.
        ...
        elif config and config.get("use_authored_locations"):
    ...
    projected["map_bounds_internal"] = bounds_internal
...
def project_clue(clue: Clue) -> dict[str, Any]:
    return {
        ...
        # The place where an inspector actually recovered a discovered clue is
        # player knowledge. Keep it separate from narrative links: a clue can
        # point at a person while still having been found somewhere concrete.
        "source_location_id": clue.discoverability.location_id,
