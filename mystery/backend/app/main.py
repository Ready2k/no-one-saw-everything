...
from .testimony import conflicts_for
from .models import (
    AccusationRequest,
    AskRequest,
    FreeTextAskRequest,
...
def _dev_map_editor_enabled() -> bool:
    """Read the operator-controlled map editor flag from the environment."""
    return os.getenv("ENABLE_DEV_MAP_EDITOR", "false").lower() == "true"
...
def get_config():
    ...
    return {
        ...
        "dev_map_editor_enabled": _dev_map_editor_enabled(),
...
class LLMSettingsUpdate(BaseModel):
    ...
    # Optional, unlike the two above, and for the same reason api_key is:
    # omitted means "leave it as it is". The Settings modal has no control for
    # Layer 7, so a plain `bool = False` would silently switch claim reasoning
    # off again every time the operator saved an unrelated field like the model
    # name — a guard that quietly turns itself off is worse than one that was
    # never enabled.
    claim_reasoning_enabled: Optional[bool] = None
...
def get_llm_settings():
    ...
    # `configured` only means "the settings are filled in" — it is computed
    # without touching the network, because get_llm_config() runs on the
    # interview hot path. On its own it made this panel show a green "Active"
    # badge for a host that does not resolve, while every rewrite in the game
    # failed and fell back to deterministic text with nothing on screen saying
    # so. Reachability is checked here, on an explicit settings read only.
    reachable: Optional[bool] = None
    health_detail: Optional[str] = None
    fallback_reason = effective.fallback_reason
    if effective.provider == "openai_compatible" and effective.base_url:
        from .llm.discovery import endpoint_reachable

        reachable, health_detail = endpoint_reachable(effective.base_url, effective.api_key)
        # `is False`, not `not reachable`: None means the probe has not come
        # back yet, and reporting "unreachable" for it would be this endpoint
        # making up an answer it does not have — the exact failure being fixed.
        if reachable is False and not fallback_reason:
            fallback_reason = "host_unreachable"

    # The other half of the same silence: `Session.llm_unavailable` latches on
    # the first provider error and drops the rest of the session to the
    # no-LLM path (spec §11's fallback contract). That is the correct
    # behaviour, but it was invisible — surfaced so the panel can say the
    # session is already degraded even if the host has since come back.
    session_degraded = False
    try:
        session_degraded = bool(session().llm_unavailable)
    except Exception:  # a settings read must never fail on session lookup
        pass
    ...
    return {
        ...
        "effective": {
            ...
            "claim_reasoning_enabled": effective.claim_reasoning_enabled,
...
def update_llm_settings(payload: LLMSettingsUpdate):
    ...
    if update.get("claim_reasoning_enabled") is None:
        update["claim_reasoning_enabled"] = bool(existing and existing.claim_reasoning_enabled)
...
def get_case():
    ...
    return {
        ...
        "weather_intensity": case.weather_intensity,
        "wind": case.wind,
...
def map_replay(
    ...
    return {
        ...
        "weather": {
            "condition": case.case.weather,
            "intensity": case.case.weather_intensity,
            "wind": case.case.wind,
        },
        "mode": mode,
        "map": {
            ...
            "zoom_image": map_definition.get("zoom_image"),
            "width": map_width,
            "height": map_height,
...
def forensic_search_tool(clue) -> str:
    """Return the player-facing collection method for a scene clue.

    Kept in the API projection so every client uses the same method for a
    clue, rather than each scene guessing independently from its title.
    """
    text = f"{clue.title} {clue.description}".lower()
    if any(word in text for word in ("blood", "stain", "damp", "smudge", "residue", "fiber", "hair", "fluid", "cleaned")):
        return "uv"
    if any(word in text for word in ("footprint", "shoe", "boot", "mud", "scratch", "mark", "impression", "scuff")):
        return "raking"
    return "lens"
...
def inspect(req: InspectRequest):
    ...
    for clue in case.clues:
        ...
        hidden_clues.append({
            ...
            "title": clue.title,
            "search_tool": forensic_search_tool(clue),
        })
...
def ask(req: AskRequest):
    ...
    # Pay for the player's likely next question while they read this answer. Runs
    # against a copy of the session and can only ever populate a rewrite cache
    # keyed on exact inputs, so it cannot change any answer — see prefetch.py.
    prefetch.warm_agent(case, sess, req.agent_id, just_asked=req.question_type)
    return interview_engine.public_ask_response(resp)
...
def challenge_suggestions(agent_id: Optional[str] = None, reveal: bool = False):
    ...
    # Testimony-based contradictions: two of a suspect's own already-heard
    # claims (or a witness's word against them) that genuinely conflict —
    # find_conflict, the same deterministic check the Challenge endpoint
    # itself uses. Previously this endpoint only ever walked authored
    # challenge_rules, so a real, player-discoverable contradiction between
    # two claims never contributed to the hint count or the reveal list —
    # only clue-backed challenges got any nudge at all. Gating is implicit:
    # a claim only enters sess.claims once the player has actually heard
    # it, which is the same discoverability guarantee required_prior_clue_ids
    # gives the clue-based path above.
    all_claims = list(sess.claims.values())
    testimony_seen: set[tuple[str, str]] = set()
    for claim in all_claims:
        if agent_id and claim.speaker_agent_id != agent_id:
            continue
        for other, _conflict in conflicts_for(claim, all_claims):
            key = tuple(sorted((claim.claim_id, other.claim_id)))
            if key in testimony_seen:
                continue
            testimony_seen.add(key)
            out.append(
                {
                    "target_agent_id": claim.speaker_agent_id,
                    "challenged_claim_id": claim.claim_id,
                    "claim_text": claim.claim_text,
                    "claim_status": claim.player_known_status,
                    "evidence_clue_id": "",
                    "evidence_claim_id": other.claim_id,
                    "evidence_title": other.claim_text,
                }
            )
    ...
    save_session(sess)
    for s in out:
        # evidence_clue_id is "" for a testimony-based suggestion (and vice
        # versa) — fall back to whichever one is actually set, so two
        # different testimony suggestions for the same challenged claim
        # don't collide on the same ("", "") key.
        key = (s["challenged_claim_id"], s["evidence_clue_id"] or s["evidence_claim_id"])
...
def reset():
    ...
    # A fresh investigation must not inherit the last run's dialogue: the cache
    # is keyed on turn inputs, and a reset session reproduces them exactly.
...
def get_dev_map_layout():
    import json
    if not _dev_map_editor_enabled():
