...
class Session:
    def __init__(self, case_id: str, player_id: str = DEFAULT_PLAYER_ID):
        ...
        # A fresh random value per investigation, mixed into every stable-hash
        # seed the deterministic dialogue layer uses (filler/action picks,
        # repeat-opener choice). Without this, every fresh investigation asking
        # the same first questions produced byte-identical deterministic-mode
        # dialogue forever — everything was keyed on (case_id, agent_id,
        # question, message count), with no notion of "which investigation is
        # this". A player who fails an accusation and starts over should not
        # read the exact same words on the retry. Persisted across save/reload
        # (same investigation keeps its seed); `reset_session` naturally mints
        # a new one because it constructs a fresh Session.
        self.session_seed: str = secrets.token_hex(4)
    ...
    def from_dict(cls, data: dict) -> "Session":
        ...
        # A pre-existing save from before this field was added has no
        # "session_seed" key — `cls(...)` above already minted a fresh one in
        # __init__, so only overwrite it when the save actually carries one.
        if data.get("session_seed"):
            s.session_seed = data["session_seed"]
        return s
