...
def answer_question(case: CaseData, session: Session, req: AskRequest) -> AskResponse:
    ...
    if rule is None:
        ...
        for ac in rule.claims:
            claim = Claim(
                ...
                # AnswerClaim.time_to was never copied here — every alibi's
                # SPAN silently degraded into a single point the moment it
                # was recorded, exactly the failure mode AnswerClaim.time_to's
                # own docstring warns against ("an alibi could only be
                # contradicted at the single minute it happened to be tagged
                # with"). Found via test_testimony.py never exercising this
                # conversion path — its tests build Claim objects directly.
                time_to=ac.time_to,
    ...
    tell_seed = f"{case.case.case_id}:{session.session_seed}:{req.agent_id}:{question_text}:{len(session.transcript_for(req.agent_id).messages)}"
    ...
    deterministic_answer = humanize_response(deterministic_answer, agent, count, seed=tell_seed, pressure=pressure)
    ...
    if config.dialogue_enabled:
        ...
        # Layer 7 (ENGINE_SPEC §9): on a turn whose answer actually rests on
        # claims, hand the model everything this agent has already told the
        # player — structured, id-tagged — instead of only the prose of the
        # last few lines. That is what lets it answer a question that spans two
        # earlier statements, and the citation it must return in exchange is
        # what the danger table is then checked against. Withheld entirely
        # unless configured on, so the default build's dialogue is unchanged.
        claim_history = None
        if config.claim_reasoning_enabled and new_claims:
            claim_history = [
                c for c in session.claims.values() if c.speaker_agent_id == req.agent_id
            ]
