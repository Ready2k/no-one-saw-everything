...
def test_telemetry_covers_inspection_and_discovery_sources(monkeypatch):
    ...
    # /api/session/log is playtest-gated (operator-only); enable it for this read.
    monkeypatch.setenv("MYSTERY_PLAYTEST_MODE", "true")
    # loc_cafe_storage deliberately exposes no magnifying-glass hotspots: its
    # clues sit in the victim's pockets and surface through the autopsy flow
    # (reveal_on=["examine_body"], skipped by /api/inspect). Searching it
    # therefore discovers nothing and never records a magnifying_glass source.
    # loc_hobbs_cafe is a room the search actually finds something in.
