...
def test_replay_shape_and_visual_metadata():
    ...
    assert "town_overworld_full_current_village_v3_neutral.avif?v=" in data["map"]["image"]
    assert data["map"]["image_tiles"]
    assert data["map"]["zoom_image_tiles"]["threshold"] == 3.2
    assert data["map"]["zoom_image"] is None
...
def test_generated_case_produces_map_replay():
    ...
    assert data["weather"] == {"condition": "clear", "intensity": "light", "wind": "calm"}
    for loc in data["locations"]:
        assert loc["map_position"] is not None
    for agent in data["agents"]:
        assert "sprite_asset" not in agent
