...
from app.llm.rewrite_cache import reset_rewrite_cache
...
def _restore_golden_case():
    ...
    # Same reasoning: the rewrite memo is process-lifetime and keyed on turn
    # inputs, so a test that stubs the client to return one thing would
    # otherwise be served an earlier test's answer for an identical turn — which
    # is exactly how three tests asserting a *fallback* started passing a cached
    # success instead.
    reset_rewrite_cache()
