...
def test_free_text_api_keeps_canned_fallback_when_dialogue_disabled(monkeypatch):
    ...
    # The deflection is now the agent's own voice archetype rather than one
    # generic string (Layer 8, dialogue_processor._ARCHETYPES) — this asserted
    # "not sure what you mean", which is only the *soft_spoken* pool's wording.
    # What the test is actually for is that an off provider still yields an
    # authored refusal and never invents case content, so assert that instead
    # of pinning one archetype's phrasing.
    #
    # ALL_CANNED_DEFLECTIONS, not just the archetype pools: an agent with no
    # recognisable voice card falls back to the temperament pools instead, and
    # 70 of the shipped cast are in that position. Checking only _ARCHETYPES
    # would pass here purely because Clara happens to have one.
    from app.dialogue_processor import ALL_CANNED_DEFLECTIONS

    message = data["fallback_message"]
    assert message in ALL_CANNED_DEFLECTIONS, f"unauthored deflection: {message!r}"
    assert "childhood" not in message.lower(), "deflection echoed the question back as content"
